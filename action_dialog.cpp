...
void prompt_for_field_data( State_menu & state, Field_spec_T const & field_spec, Lib_tty::Key_char_i18ns const & current_value, std::string const & progress_message ) {
    ...
    Lib_tty::Key_char_i18ns current_value_echo {};
...
template< typename T >
Lib_tty::Key_char_i18ns existing_o_default( std::optional<T> const & default_value, std::optional<Lib_tty::Key_char_i18ns> const & existing_value ) {
...
template< typename T >
std::optional<Lib_tty::Key_char_i18ns> to_string_typed_value( T typed_value ) {
...
/****** returns true if we need more input from user
 */
bool process_intra_field_nav( Lib_tty::FieldIntraNav const intra_f_nav, Lib_tty::Key_char_i18ns & /*INOUT*/ current_value ) {
    switch ( intra_f_nav ) {
...
bool is_lone_negative_sign_o_0_length ( Lib_tty::Key_char_i18ns const & current_value, std::string /*OUT*/ & deficiency_message  ) {
...
bool process_field_completion_nav( State_menu						state,
                                   ...
                                   Lib_tty::Key_char_i18ns const			& current_value,
    ...
    Lib_tty::Key_char_i18ns 	fixed_current_value 	{};
...
bool process_valid_values_nav( Itr_type 				& cur_itr, 					// todo: should these be copies so I don't mess up caller?
                               ...
                               Lib_tty::Key_char_i18ns 		& /*OUT*/ current_value,
                               Lib_tty::Hot_key const 			& hot_key,
...
std::optional<std::string> check_numeric_invalidity( VSpec_type const 		 & validity_spec,
                                          Lib_tty::Key_char_i18ns const & current_value ) {  // todo: should be able to do this without templates, using typeid()
...
bool process_numeric_decorators( size_t const 				length,
                                 Lib_tty::Key_char_i18ns const 	& Key_char_i18ns,
                                 IO_field_spec_variant const& field_spec ,
                                 std::string 				& deficiency_message ) {
    assert( Key_char_i18ns.length() == 1 );// allow comma,  todo: anywhere: not good.  study locales and number punctiation.
    if ( ! check_for_numeric_type( field_spec ) )
        return false;
    ...
    if ( std::isdigit( static_cast<unsigned char>( Key_char_i18ns.at(0) )))
        is_allowed_digit = true;
    else
    {
        if ( Key_char_i18ns.at(0) == '-' || Key_char_i18ns.at(0) == '+' ) {
            ...
            if ( Key_char_i18ns.at(0) == '.' ) {  // todo: complete this: fix for locale of '.' or ','.
...
Value_nup input_field_response( State_menu 						      & state,
    ...
    while ( ! gotten_field_data.has_value() ) {
        ...
        auto [Key_char_i18ns, hot_key, file_status] =
        ...
        if ( check_prohibited_characters( validity_spec, Key_char_i18ns.at(0), deficiency_message ) )
            continue;
        if ( check_max_input_length( field_spec, current_value, deficiency_message )) //                        is_user_entered_a_valid_char ))
            continue;
        current_value += Key_char_i18ns;
...
//Value_nup input_field_response( State_menu & state, IO_field_spec_character 	const & field_spec, std::optional<Lib_tty::Key_char_i18ns> const & existing_value) {
//Value_nup input_field_response( State_menu & state, IO_field_spec_uint64 		const & field_spec, std::optional<Lib_tty::Key_char_i18ns> const & existing_value) {
//Value_nup input_field_response( State_menu & state, IO_field_spec_time_point 	const & field_spec, std::optional<Lib_tty::Key_char_i18ns> const & existing_value) {
//Value_nup input_field_response( State_menu & state, IO_field_spec_tm 			const & field_spec, std::optional<Lib_tty::Key_char_i18ns> const & existing_value) {
Value_nup input_field_response( State_menu 								& state,
...
Value_nup input_field_response( State_menu & state, IO_field_spec_integer 		const & field_spec, std::optional<Lib_tty::Key_char_i18ns> const & existing_value ) {
    exclude_disallowed_fields( field_spec ); // todo: complete this: field_spec.lengths_input.max don't think we can/need to enforce min for an integer.
    std::string 		deficiency_message 		{ field_spec.full_description };
    bool 				is_editing_mode_insert 	{true};
    bool				is_user_entered_a_valid_char {false};
    ...
    //Lib_tty::Key_char_i18ns 	kludge 					{}; // std::optional<Lib_tty::Key_char_i18ns> default_value { /* retrieve_default( current_valid_value_itr, valid_value_set.end(), kludge )*/ std::nullopt };
    auto const 			default_typed_value 	{ retrieve_default( current_valid_value_itr, valid_value_set.end() ) };
    std::optional<Lib_tty::Key_char_i18ns>
                        default_value 			{ to_string_typed_value( default_typed_value )};
    Lib_tty::Key_char_i18ns 	current_value 			{ existing_o_default( default_value, existing_value ) };  // we build up the value we want to return character by character.
    pagination_reset( state, { 1, 40 } ); cout << '\n';  // create a new line on display and use that one line again and again as we reprompt for every character typed.// todo: complete this, is this reset true?  20 is an estimate!
    std::optional<Value_nup> gotten_field_data			{std::nullopt};
    while ( !gotten_field_data.has_value() ) {
        prompt_for_field_data( state, field_spec, current_value, deficiency_message );
        deficiency_message = field_spec.full_description;  // let the user see the hint, unless of course we find a deficiency below.
        auto [Key_char_i18ns, hot_key, file_status] =  Lib_tty::get_kb_keystrokes_raw( 1,
                                                                                 false,
                                                                                 field_spec.prompt_field_spec.echo_mode==IO_field_echo_mode::normal ? true : false,
                                                                                 validity_spec.base.is_strip_control_chars
                                                                                 );
        ...
        if ( check_prohibited_characters( validity_spec, Key_char_i18ns.at(0), deficiency_message ) )
            continue;
        if ( process_numeric_decorators( current_value.length(), Key_char_i18ns, field_spec, deficiency_message ))
            continue;
        if ( check_max_input_length( field_spec, current_value, deficiency_message ) )
            continue;
        current_value += Key_char_i18ns;
...
Value_nup input_field_response( State_menu & state, IO_field_spec_decimal 		const & field_spec, std::optional<Lib_tty::Key_char_i18ns> const & existing_value ) {
    exclude_disallowed_fields( field_spec );            // todo: complete this: field_spec.lengths_input.max don't think we can/need to enforce min for an integer.
    std::string 		deficiency_message 		{ field_spec.full_description };
    bool 				is_editing_mode_insert 	{true};
    bool				is_user_entered_a_valid_char {false};
    ...
    while ( !gotten_field_data.has_value() ) {
        ...
        if ( check_prohibited_characters( validity_spec, Key_char_i18ns.at(0), deficiency_message ) )
            continue;
        //if ( process_numeric_decorators( current_value.length(), Key_char_i18ns, deficiency_message, false ))
        if ( process_numeric_decorators( current_value.length(), Key_char_i18ns, field_spec, deficiency_message ))
            continue;
        if ( check_max_input_length( field_spec, current_value, deficiency_message ) )
            continue;
...
//Value_nup input_field_response( State_menu & state, IO_field_spec_scientific	const & field_spec, std::optional<Lib_tty::Key_char_i18ns> const & existing_value ) // todo: this.
...
/* Value_nup input_dialog_response( State_menu & state, std::string const & prompt, ValidityCriteriaDialog const & validity ) {
    while ( true ) {
        ...
        auto [Key_char_i18ns, hot_key, file_status] = get_value_raw();  // todo: specify size??

        std::string kludge = Key_char_i18ns; // todo: TODO why do I need this or get compile error?

        switch (validity.default_handling) {
            case ValidityHandlingDialog::default_is_to_notify:
                return { {}, hot_key, {} }; // no problem, the end value will just be "".
            case ValidityHandlingDialog::default_is_to_approve:
                if ( Key_char_i18ns.length() == 0 )
            ...
            case ValidityHandlingDialog::default_is_provided:
                if ( Key_char_i18ns.length() == 0 ) {
                    Key_char_i18ns = validity.value_default_provided;
                    return { Key_char_i18ns, hot_key, {} };
                }
                break;
            case ValidityHandlingDialog::check_valid_values:
            case ValidityHandlingDialog::no_specific_value_checking:
        ...
        switch (validity.default_handling) {
            case ValidityHandlingDialog::check_valid_values:
                ...
                else { // auto found2 = std::find_if( valid.values.begin(), valid.values.end(), "junk" //Key_char_i18ns//);  // todo: TODO why on earth do I get a strange iteration compile error on this?
            ...
            case ValidityHandlingDialog::default_is_provided:
                if (( validity.lengths.min || validity.lengths.max )   // if we have them then check them.
                       &&
                    ( Key_char_i18ns.length() < validity.lengths.min || Key_char_i18ns.length() > validity.lengths.max ))
                {
                    user_error_prompt = "Value is too ";
                    pagination( state, {1, user_error_prompt.length() + 5 + 1});
                    cout << user_error_prompt << (Key_char_i18ns.length() < validity.lengths.min ? "long" : "short");
                    cout << ".";
                    break;
                }
                else
...
/* InteractionResult action_dialog_modal_vals(State_menu & state, std::string const & prompt, ValidityCriteriaDialog const & validity) {  // different from process_menu() because we don't expect a major change in the flow of the program, ie. yes/no for a data save to file. BUT yes we do want to support _some_ hot_keys!
    ...
    std::optional<Lib_tty::Key_char_i18ns>
        Key_char_i18ns 	                = std::get<std::optional<Lib_tty::Key_char_i18ns>>(  value_plus );
        ...
        InteractionResult operator() (State_menu & state, IO_field_spec_bool 			& field_spec ) {
            ...
            while ( true ) {
                ...
                if ( is_store_value_intent( irn ) ) {
                    if ( !value_nup.Key_char_i18ns_opt.has_value() ) {
                        field_spec.clear_data_value();
                        return { value_nup.hot_key, {}, {}, {}, irn };
                    } else {
                        std::decay_t<decltype (field_spec.data_location)> typed_value {false};
                        if ( auto const itr = VALUE_DEFAULT_APPROVE_EQUIVALENTS.find( value_nup.Key_char_i18ns_opt.value() ); itr != VALUE_DEFAULT_APPROVE_EQUIVALENTS.end() )
                            typed_value = true;
                        else if ( auto const itr = VALUE_DEFAULT_DENY_EQUIVALENTS.find( value_nup.Key_char_i18ns_opt.value() ); itr != VALUE_DEFAULT_DENY_EQUIVALENTS.end() )
                            typed_value = false;
                        else
                            assert(false);
                        if ( std::optional<ValidationFieldError> vfr = field_spec.validate_data( typed_value ); !vfr.has_value() ) {
                            ...
                            // return { value_nup.hot_key, {}, value_nup.Key_char_i18ns_opt.value(), {}, irn }; // return GOOD Typed VALUE
            ...
            while ( true ) {
                ...
                if ( is_store_value_intent( irn ) ) {
                    if ( !value_nup.Key_char_i18ns_opt.has_value() ) {
                        field_spec.clear_data_value();
                        return { value_nup.hot_key, {}, {}, {}, irn };
                    } else if ( std::optional<ValidationFieldError> vfr = field_spec.validate_data( value_nup.Key_char_i18ns_opt.value() ); !vfr.has_value() ) {
                        field_spec.setData_value( value_nup.Key_char_i18ns_opt.value() );
                        return { value_nup.hot_key, {}, value_nup.Key_char_i18ns_opt.value(), {}, irn }; // return GOOD Typed VALUE { Data_type_variant {}, InteractionResultError {}, InteractionResultNav::retain_menu };
                    } else {
            ...
            while ( true ) {
                ...
                if ( is_store_value_intent( irn ) ) {
                    if ( !value_nup.Key_char_i18ns_opt.has_value() ) {
                        ...
                        try {
                            typed_value = stoi( value_nup.Key_char_i18ns_opt.value() );
            ...
            while ( true ) {
                ...
                if ( is_store_value_intent( irn ) ) {
                    if ( !value_nup.Key_char_i18ns_opt.has_value() ) {
                        ...
                        std::decay_t<decltype (field_spec.data_location)> typed_value;
                        try {
                            typed_value = stod( value_nup.Key_char_i18ns_opt.value() );
                        } catch (std::invalid_argument const &) {
                            field_spec.clear_data_value();
