...
/* Now this is provided by lib_tty.h
enum class Lib_tty::InteractionIntentNav {  // TODO: TODO: would this be more compact if "char" or "byte"? What are downsides other than performance??
...
struct InteractionResult {   /// ALL about what happened upon user data entry, between the user and the computer via the keyboard
        Lib_tty::Hot_key_row				hot_key_row		{};  /// Interaction within interaction requires this value.  ie. help within a field interaction.
        ...
        Lib_tty::InteractionIntentNav  	navigation 	{Lib_tty::InteractionIntentNav::no_result};  /// hint that is passed to the caller on user's intent for future processing, or that which is required due to an error.

        std::string to_string(){  /// for debugging  TODO: finish this
            std::string s { hot_key_row.to_string() + ", " +
                            std::to_string(index) };
            return s;
        };
};
